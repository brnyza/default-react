import React from 'react'
import { useState } from 'react'
import { MdAdd } from 'react-icons/md'

const Button: React.FC<any> = ({children, className, noStyle, hint, ...props}) => {

  const [showHint, setShowHint] = useState(false)

  return (
    <button 
      className={` 
        ${ !noStyle ? 'bg-red-500 text-white w-24 h-10 rounded' : ' ' }  
        cursor-pointer relative flex justify-center items-center ${className}`} 
      {...props}
      onMouseOver={() => setShowHint(true)}
      onMouseLeave={() => setShowHint(false)}
      >
        {
          (showHint && hint) &&
          <div className="pointer-events-none absolute -top-10 w-max bg-black text-white px-2 py-1 flex justify-center bg-opacity-70 rounded animate-arrive">
            <div className="absolute -bottom-5 w-5 h-5 overflow-hidden">
              <div className="absolute bg-black bg-opacity-70 w-5 h-5 -top-4  transform rotate-45"  /> 
            </div>
            { hint }
          </div>
        }
          {children}
    </button>
)
}

const BtnNovo: React.FC = () => {
  return (
    <Button className="bg-green-500 flex gap-2 items-center justify-center">
      Novo
      <MdAdd className="text-xl"/>
    </Button>
  )
}

export {Button, BtnNovo}